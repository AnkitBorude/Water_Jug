		...
		public Jug.State checkRules(Jug.State state, int rule_no) {
			...
			case 1:
				if (state.getCurrentJug1State() < Jug.Jug1MaxSize) {
					return new Jug.State(Jug.Jug1MaxSize, state.getCurrentJug2State());// fill the x
				} else {
					return null;
				}
			case 2:
				if (state.getCurrentJug2State() < Jug.Jug2MaxSize) {
					return new Jug.State(state.getCurrentJug1State(), Jug.Jug2MaxSize);// fill the y
				} else {
					return null;
				}
			case 3:
				if (state.getCurrentJug1State() > 0) {
					return new Jug.State(0, state.getCurrentJug2State());// fill the x
				} else {
					return null;
				}
			case 4:
				if (state.getCurrentJug2State() > 0) {
					return new Jug.State(state.getCurrentJug1State(), 0);// fill the x
				} else {
					return null;
				}
			case 5:
			...
			case 7:
				if (state.getCurrentJug2State() > 0 && (state.getSum() <= jug.getJug1MaxSize())) {
					return new Jug.State(state.getSum(), 0);
				} else {
					return null;
				}
			case 8:
				if (state.getCurrentJug1State() > 0 && (state.getSum() <= jug.getJug2MaxSize())) {
					return new Jug.State(0, state.getSum());
				} else {
					return null;
				}
			default:
